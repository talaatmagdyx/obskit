...
def with_observability(
    ...
    **default_context: Any,
    ...
    sample_rate : float, default=1.0
        Fraction of calls that go through the full observability pipeline.
        ``1.0`` = always observe (default, no change in behaviour).
        ``0.01`` = observe 1 % of calls, skip logging/metrics for the rest.
        Non-sampled calls still execute the wrapped function normally —
        only the instrumentation is skipped.

    high_throughput : bool, default=False
        When ``True``, replace the synchronous pipeline with the high-throughput
        pipeline (AsyncLogRing + ThreadLocalAggregator).  This reduces per-call
        overhead from ~20–30 µs to ~100–150 ns at the cost of best-effort
        semantics: logs are buffered and may be dropped if the ring is full;
        metrics are flushed to Prometheus every ~1 s instead of immediately.
        ``log_start``, ``threshold_ms``, and ``track_metrics=False`` are ignored
    ...
    **default_context : Any
    ...
    def decorator(func: Callable[P, Awaitable[T]]) -> Callable[P, Awaitable[T]]:
        ...
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
            ...
            # =================================================================
            # Step 1: High-throughput path (AsyncLogRing + ThreadLocalAggregator)
            # =================================================================
            if high_throughput:
                start_time = time.perf_counter()
                try:
            ...
            # =================================================================
            # Step 2: Build context dictionary (standard path only)
            # =================================================================
            ...
            # =================================================================
            # Step 3: Set up correlation context
            # =================================================================
...
def with_observability_sync(
    ...
    **default_context: Any,
    ...
    sample_rate : float, default=1.0
        ...
        Non-sampled calls still execute the wrapped function normally.
    high_throughput : bool, default=False
        When ``True``, use AsyncLogRing + ThreadLocalAggregator instead of the
        synchronous pipeline.  Best-effort semantics apply (see ``with_observability``).
    **default_context : Any
