...
class HealthRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Check custom handlers first (snapshot under lock to avoid race conditions)
        with _server_lock:
            handler_fn = _custom_handlers.get(path)
...
def start_health_server(
    ...
    host : str
        Host to bind to (default: '127.0.0.1'). Use '0.0.0.0' only behind a
        firewall or in environments where the endpoint is not externally reachable.
    daemon : bool
