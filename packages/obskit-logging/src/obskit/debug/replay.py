...
class FileStorage(RequestCaptureStorage):
    ...
    def __init__(
        ...
        resolved = base_path or str(Path(tempfile.gettempdir()) / "obskit_captures")
...
class RequestCapture:
    ...
    Example:
        capture = RequestCapture(
            storage=FileStorage(),  # defaults to <tempdir>/obskit_captures
    ...
    async def replay(
        ...
        # Dry-run: return metadata without executing (no function needed)
        if dry_run:
            result["success"] = True
        ...
        # Find function (only needed for actual execution)
        if not func:
