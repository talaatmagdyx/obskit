...
class TestAuthenticatedMetricsHandler:
    ...
    def _create_handler_instance(self, auth_token=None, path="/metrics", auth_header=None):
        ...
        original_send_response = lambda code: setattr(handler, '_response_code', code)
        original_send_header = lambda k, v: handler._response_headers.append((k, v))
        original_end_headers = lambda: None
