...
PREDICTION_VALUE = Gauge(
    "resource_predictor_forecast",
    "Predicted resource value",
    ["resource", "hours_ahead"]
)
...
class Forecast:
    ...
    threshold: float
    hours_until_threshold: Optional[float]
...
class ResourcePredictor:
    ...
    def record(self, resource: str, value: float, timestamp: Optional[datetime] = None):
        """
        ...
        with self._lock:
            ...
            self._data[resource] = [
                p for p in self._data[resource]
    ...
    def predict(
        ...
        trend_value = 1 if trend.trend_direction == "increasing" else (-1 if trend.trend_direction == "decreasing" else 0)
    ...
    def _calculate_trend(self, points: List[DataPoint]) -> Optional[TrendAnalysis]:
        """Calculate trend using linear regression."""
        if len(points) < 2:
            return None
    ...
    def clear(self, resource: Optional[str] = None):
        """Clear data."""
        with self._lock:
            if resource:
