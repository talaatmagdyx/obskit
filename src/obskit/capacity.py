...
CAPACITY_CURRENT = Gauge(
    "capacity_current_usage",
...
class Resource:
    ...
    max_value: float
    unit: str = "units"
    growth_rate_per_month: Optional[float] = None
...
class CapacityPlan:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "action_required_by": self.action_required_by.isoformat() if self.action_required_by else None,
...
class CapacityPlanner:
    ...
    def add_resource(
        ...
        with self._lock:
            ...
            self._growth_data[name].append(GrowthDataPoint(
    ...
    def update_resource(self, name: str, current_value: float):
        ...
        with self._lock:
            ...
            self._growth_data[name] = [
                p for p in self._growth_data[name]
    ...
    def calculate_growth_rate(self, name: str) -> Optional[float]:
    ...
    def project_resource(
        ...
        if growth_rate > 0:
            ...
            if resource.current_value < warning_value:
                months = math.log(warning_value / resource.current_value) / math.log(1 + growth_rate)
                days_until_warning = int(months * 30)
            
            if resource.current_value < critical_value:
                months = math.log(critical_value / resource.current_value) / math.log(1 + growth_rate)
                days_until_critical = int(months * 30)
            
            if resource.current_value < resource.max_value:
                months = math.log(resource.max_value / resource.current_value) / math.log(1 + growth_rate)
        ...
        recommendation = self._generate_recommendation(
            resource, projected, days_until_exhaustion
        )
        ...
        for name in resource_names:
            ...
            if proj:
                ...
                if proj.days_until_exhaustion and proj.days_until_exhaustion < months_ahead * 30:
                    ...
                    action_date = datetime.utcnow() + timedelta(days=proj.days_until_exhaustion - 30)
