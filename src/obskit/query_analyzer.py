...
SLOW_QUERIES_TOTAL = Counter(
    "query_analyzer_slow_queries_total",
    "Queries identified as slow",
    ["database", "query_type"]
)

MISSING_INDEX_DETECTED = Counter(
    "query_analyzer_missing_index_total",
    "Queries with missing index",
    ["database", "table"]
)
...
class QueryAnalysis:
    ...
    plan_tree: Optional[QueryPlanNode] = None
...
class QueryAnalyzer:
    ...
    def _extract_tables(self, query: str) -> List[str]:
        ...
        insert_match = re.findall(r'\bINSERT\s+INTO\s+([a-zA-Z_][a-zA-Z0-9_]*)', query, re.IGNORECASE)
    ...
    def get_analysis(self, query_hash: str) -> Optional[QueryAnalysis]:
