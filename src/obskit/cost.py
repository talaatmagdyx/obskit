...
COST_API_CALLS = Counter(
    ...
    "External API calls",
    ["service", "tenant_id", "api", "method"]
)

COST_STORAGE = Gauge(
    "cost_storage_bytes",
    "Storage used",
    ["service", "tenant_id", "storage_type"]
)

COST_NETWORK_BYTES = Counter(
    "cost_network_bytes_total",
    "Network bytes transferred",
    ["service", "tenant_id", "direction"]
)

COST_UNITS = Counter(
    "cost_units_total",
    "Abstract cost units",
    ["service", "tenant_id", "resource_type"]
)
...
class CostTracker:
    ...
    def track_cpu(
        ...
        try:
            ...
            self._usage[tenant_id].cost_units += (
                duration * self.cost_rates["cpu_second"]
            )
    ...
    def track_memory_usage(
        ...
        self._usage[tenant_id].tenant_id = tenant_id
        self._usage[tenant_id].memory_bytes = max(
            self._usage[tenant_id].memory_bytes,
            bytes_used
        )
    ...
    def track_api_call(
        ...
        self._usage[tenant_id].cost_units += (
            cost_units * self.cost_rates["api_call"]
        )
    ...
    def calculate_cost(
        ...
        return {
            ...
            "network": (
                (usage.network_bytes_in + usage.network_bytes_out) / (1024 ** 3)
    ...
    def reset_usage(self, tenant_id: Optional[str] = None):
        """
...
def track_cost(
    ...
    operation_arg: Optional[str] = None
) -> Callable[[F], F]:
    """
