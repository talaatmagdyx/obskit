...
DEPENDENCY_LATENCY = Gauge(
    "dependency_graph_latency_ms",
    "Dependency latency in milliseconds",
    ["service", "dependency"]
)

DEPENDENCY_CALLS = Counter(
    "dependency_graph_calls_total",
    "Total calls to dependency",
...
class DependencyGraph:
    ...
    def record_call(
        ...
        with self._lock:
            ...
            # Update error rate
            node.error_rate = (
                node.failed_calls / node.total_calls
                if node.total_calls > 0 else 0.0
            )
    ...
    def _update_dependency_count(self):
        ...
        with self._lock:
            ...
            for dep_type, count in counts.items():
                DEPENDENCY_COUNT.labels(
                    service=self.service_name,
                    dependency_type=dep_type
                ).set(count)
    
    def get_dependency(self, name: str) -> Optional[DependencyNode]:
