...
class DependencyGraph:
    ...
    def __init__(
        ...
        self._lock = threading.RLock()  # RLock: record_call→add_dependency and get_visualization_data→get_critical_path both re-enter
    ...
    def remove_dependency(self, name: str):
        ...
        with self._lock:
            ...
            edge_key = f"{self.service_name}->{name}"
            if edge_key in self._edges:  # pragma: no branch
    ...
    def record_call(
        ...
        with self._lock:
            ...
            if edge_key in self._edges:
                ...
                if sorted_samples:  # pragma: no branch
...
def get_dependency_graph(service_name: str, **kwargs) -> DependencyGraph:
    ...
    if service_name not in _graphs:
        with _graph_lock:
            if service_name not in _graphs:  # pragma: no cover
                _graphs[service_name] = DependencyGraph(service_name, **kwargs)  # pragma: no cover
