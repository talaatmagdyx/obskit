...
class Incident:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "time_to_detect_seconds": self.time_to_detect.total_seconds() if self.time_to_detect else None,
            "time_to_mitigate_seconds": self.time_to_mitigate.total_seconds() if self.time_to_mitigate else None,
            "time_to_resolve_seconds": self.time_to_resolve.total_seconds() if self.time_to_resolve else None,
        }
...
class IncidentTimeline:
        ...
        with self._lock:
            ...
            if status == IncidentStatus.IDENTIFIED and not self.incident.identified_at:
                ...
                if self.incident.time_to_resolve:
                    ...
                    ).observe(self.incident.time_to_resolve.total_seconds())
                
                if self.incident.total_duration:
                    INCIDENT_DURATION.labels(
                        severity=self.incident.severity
                    ).observe(self.incident.total_duration.total_seconds())
    ...
    def generate_postmortem(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "severity": self.incident.severity,
            "duration": str(self.incident.total_duration) if self.incident.total_duration else "Ongoing",
            ...
            "metrics": {
                "time_to_detect": str(self.incident.time_to_detect) if self.incident.time_to_detect else None,
                "time_to_mitigate": str(self.incident.time_to_mitigate) if self.incident.time_to_mitigate else None,
                "time_to_resolve": str(self.incident.time_to_resolve) if self.incident.time_to_resolve else None,
            },
...
class IncidentManager:
    ...
    def get_incident(self, incident_id: str) -> Optional[IncidentTimeline]:
