...
DLQ_MESSAGES_TOTAL = Counter(
    ...
    ["dlq_name", "original_queue", "reason"]
)
...
class DLQTracker:
    ...
    def __init__(
        ...
        on_threshold_exceeded: Optional[Callable[[str, int], None]] = None,
    ):
    ...
    def track_message_sent(
        ...
        **metadata
        ...
        DLQ_MESSAGE_AGE.labels(
            dlq_name=self.dlq_name,
            original_queue=original_queue
        ).observe(message_age_seconds)
    ...
    def track_processing(
        ...
        try:
            ...
            DLQ_PROCESSING_TOTAL.labels(
                dlq_name=self.dlq_name,
                status=status
            ).inc()
            
            DLQ_PROCESSING_LATENCY.labels(
                dlq_name=self.dlq_name
            ).observe(duration)
            
            DLQ_REPROCESSED_TOTAL.labels(
                dlq_name=self.dlq_name,
    ...
    def get_stats(self) -> DLQStats:
        ...
        with self._lock:
            ...
            for msg in self._messages.values():
                ...
                messages_by_queue[msg.original_queue] = messages_by_queue.get(msg.original_queue, 0) + 1
