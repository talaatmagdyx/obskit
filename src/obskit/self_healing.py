...
HEALING_TRIGGERS_EVALUATED = Counter(
    "self_healing_triggers_evaluated_total",
...
HEALING_ACTIVE_TRIGGERS = Gauge(
    "self_healing_active_triggers",
    "Number of active triggers"
)

HEALING_COOLDOWN_ACTIVE = Gauge(
    "self_healing_cooldown_active",
    "Triggers currently in cooldown",
    ["trigger"]
)
...
class HealingTrigger:
    ...
    last_result: Optional[HealingResult] = None
...
class SelfHealingEngine:
    ...
    def evaluate(self, trigger_name: Optional[str] = None) -> List[HealingEvent]:
        """
        ...
        with self._lock:
            triggers = [self._triggers[trigger_name]] if trigger_name else list(self._triggers.values())
    ...
    def _evaluate_trigger(self, trigger: HealingTrigger) -> Optional[HealingEvent]:
        ...
        with self._lock:
            ...
            recent_executions = [
                t for t in self._hourly_executions.get(trigger.name, [])
                if t > hour_ago
            ]
        ...
        try:
            ...
            HEALING_ACTIONS_EXECUTED.labels(
                trigger=trigger.name,
                result=result.value
            ).inc()
    ...
    def get_trigger(self, name: str) -> Optional[HealingTrigger]:
