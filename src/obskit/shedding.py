...
SHEDDER_REQUESTS_TOTAL = Counter(
    "load_shedder_requests_total",
    "Total requests evaluated",
    ["shedder_name", "decision"]
)

SHEDDER_SHED_RATE = Gauge(
    "load_shedder_shed_rate",
    "Current shedding rate (0-1)",
    ["shedder_name"]
)

SHEDDER_QUEUE_SIZE = Gauge(
    "load_shedder_queue_size",
    "Current queue size",
    ["shedder_name"]
)

SHEDDER_LATENCY_MS = Gauge(
    "load_shedder_latency_ms",
    "Current observed latency",
    ["shedder_name"]
)

SHEDDER_LOAD_LEVEL = Gauge(
    "load_shedder_load_level",
...
class LoadShedder:
    ...
    def __init__(
        ...
        adaptive: bool = True,
        on_shed: Optional[Callable[[Priority], None]] = None,
    ):
        self.name = name
    ...
    def _evaluate_shed_rate(self):
        ...
        if load_level > 0.8:
            ...
            new_rate = max(
                self.config.min_shed_rate,
                self._current_shed_rate * self.config.ramp_down_factor
            )
        else:
