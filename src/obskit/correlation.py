...
def get_custom_context() -> Dict[str, Any]:
    ...
    return _custom_context.get().copy()


def set_custom_value(key: str, value: Any):
    """Set a custom context value."""
    ctx = _custom_context.get().copy()
    ctx[key] = value
    _custom_context.set(ctx)
...
class CorrelationManager:
    ...
    @staticmethod
    def propagate_to_headers(headers: Optional[Dict[str, str]] = None) -> Dict[str, str]:
        """
    ...
    def extract_from_headers(headers: Dict[str, str]) -> Dict[str, Any]:
        ...
        for header_name, key in header_mapping.items():
            ...
            value = (
                ...
                headers.get(header_name.lower()) or
                headers.get(header_name.lower().replace("-", "_"))
            )
            if value:
                context[key] = value
    ...
    def propagate_to_message(
        ...
        message[headers_key] = CorrelationManager.propagate_to_headers(
            message[headers_key]
        )
        return message
