...
FEATURE_STATE = Gauge(
    "degradation_feature_state",
    "Feature state (1=enabled, 0=degraded)",
    ["feature"]
)

DEGRADATION_LEVEL = Gauge(
    "degradation_level",
    "Current degradation level (0-100)",
    ["service"]
)
...
FALLBACK_CALLS = Counter(
    "degradation_fallback_calls_total",
    "Calls to fallback behavior",
    ["feature"]
)
...
class DegradationManager:
    ...
    def set_level(self, level: DegradationLevel | int, reason: Optional[str] = None):
        """
    ...
    def degrade_feature(self, feature_name: str, reason: Optional[str] = None):
