...
ANOMALIES_DETECTED = Counter(
    "root_cause_anomalies_total",
    "Total anomalies detected",
    ["component", "severity"]
)

ANALYSIS_PERFORMED = Counter(
    "root_cause_analysis_total",
    "Total root cause analyses",
    ["result"]
)

ACTIVE_ANOMALIES = Gauge(
    "root_cause_active_anomalies",
    ...
    def analyze(
        ...
        with self._lock:
            ...
            recent_anomalies = [
                ...
                if a.timestamp >= cutoff and not a.resolved
            ]
        ...
        affected_components = list(set(a.component for a in recent_anomalies))
        ...
        if matched_rules:
            ...
            if len(recent_anomalies) == 1:
                ...
                probable_cause = f"Isolated {anomaly.anomaly_type.value} issue in {anomaly.component}"
                confidence = 0.5
                ...
                if AnomalySeverity.CRITICAL in severities:
                    ...
                    probable_cause = f"Critical issue in {critical[0].component} with cascade effects"
                    confidence = 0.6
                    suggestions = [
        ...
        ANALYSIS_PERFORMED.labels(
            result="cause_found" if probable_cause else "no_cause"
        ).inc()
