...
ALERT_GROUP_SIZE = Gauge(
    "alert_group_occurrences",
    "Number of occurrences in alert group",
    ["alert_name", "severity"]
)
...
class DeduplicationConfig:
    ...
    severity_cooldowns: Dict[str, int] = field(default_factory=lambda: {
        "critical": 5,
        "warning": 15,
        "info": 60,
    })
...
class AlertDeduplicator:
    ...
    def __init__(
        ...
        on_aggregated: Optional[Callable[[AlertRecord], None]] = None,
    ):
            ...
            if fingerprint in self._suppression_windows and not force:
                ...
                if now < suppression_end:
                    ...
                    if fingerprint in self._alerts:
                        ...
                        ALERT_GROUP_SIZE.labels(
                            alert_name=alert_name,
                            severity=severity
                        ).set(self._alerts[fingerprint].count)
                        
                        if self.on_aggregated:
                            self.on_aggregated(self._alerts[fingerprint])
                    
                    ALERTS_DEDUPLICATED.labels(
                        alert_name=alert_name,
                        severity=severity
                    ).inc()
    ...
    def get_active_alerts(self) -> List[AlertRecord]:
        ...
        with self._lock:
            return [
                record for record in self._alerts.values()
    ...
    def get_suppressed_alerts(self) -> Dict[str, datetime]:
        ...
        with self._lock:
            return {
                fp: end_time
                for fp, end_time in self._suppression_windows.items()
                if end_time > now
            }
    ...
    def cleanup(self):
        ...
        with self._lock:
            # Clean old alerts
            old_fingerprints = [
                fp for fp, record in self._alerts.items()
                if record.last_seen < window_start
            ]
            ...
            expired = [
                fp for fp, end_time in self._suppression_windows.items()
                if end_time < now
            ]
            for fp in expired:
