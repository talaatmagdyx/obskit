...
BUDGET_STATUS = Gauge(
    ...
    "Budget status (1=healthy, 0=violated)",
    ["budget_name"]
)

BUDGET_UTILIZATION = Gauge(
    "performance_budget_utilization",
...
class PerformanceBudget:
    ...
    def __init__(
        ...
        throughput_max_rps: Optional[float] = None,
        window_seconds: int = 60,
        on_violation: Optional[Callable[[str, str, float], None]] = None
    ):
        """
    ...
    def _calculate_percentile(self, p: float) -> Optional[float]:
        """Calculate latency percentile."""
        if not self._latencies:
            return None
        
        sorted_latencies = sorted(l[1] for l in self._latencies)
    ...
    def get_current_metrics(self) -> Dict[str, Optional[float]]:
        ...
        return {
            ...
            "error_rate_percent": (
                len(self._errors) / len(self._requests) * 100
        ...
        for metric_name, threshold, op in checks:
            ...
            if threshold > 0:
                ...
                BUDGET_UTILIZATION.labels(
                    budget_name=self.name,
                    metric=metric_name
                ).set(min(utilization, 200))  # Cap at 200%
...
class BudgetManager:
    ...
    def get(self, name: str) -> Optional[PerformanceBudget]:
    ...
    def get_exceeded_budgets(self) -> List[str]:
        ...
        return [
            name for name, budget in self._budgets.items()
            if budget.is_exceeded()
        ]
