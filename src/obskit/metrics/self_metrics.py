...
class ObskitSelfMetrics:
    ...
    def __init__(self) -> None:
        ...
        # Register this instance so reset_self_metrics() can find it later.
...
def reset_self_metrics() -> None:
    ...
    Unregisters all Prometheus collectors created by ANY ``ObskitSelfMetrics``
    instance (including those created directly in tests, not just the singleton)
    so that the next ``ObskitSelfMetrics()`` call can re-register them without
    hitting ``ValueError: Duplicated timeseries in CollectorRegistry``.
    """
    global _self_metrics, _all_instances
    with _self_metrics_lock:
        if PROMETHEUS_AVAILABLE and _all_instances:
            try:
                from prometheus_client import REGISTRY

                for instance in _all_instances:
                    for collector in (
                        instance._queue_depth,
                        instance._queue_capacity,
                        instance._dropped_total,
                        instance._errors_total,
                        instance._info,
                    ):
