...
try:
    ...
    PROMETHEUS_AVAILABLE = False  # pragma: no cover
    Counter = None  # type: ignore[assignment,misc]  # pragma: no cover
    Gauge = None  # type: ignore[assignment,misc]  # pragma: no cover
...
# Metrics for monitoring cardinality protection — only created when
# prometheus_client is available (installed via obskit[metrics]).
if PROMETHEUS_AVAILABLE:
    CARDINALITY_REJECTIONS: Counter = Counter(
        "obskit_cardinality_rejections_total",
        "Total number of label values rejected due to cardinality limits",
        ["label_name"],
    )

    CARDINALITY_CURRENT: Gauge = Gauge(
        "obskit_cardinality_current",
        "Current number of unique values tracked for each label",
        ["label_name"],
    )

    CARDINALITY_LIMIT: Gauge = Gauge(
        "obskit_cardinality_limit",
        "Configured cardinality limit for each label",
        ["label_name"],
    )
else:  # pragma: no cover
    CARDINALITY_REJECTIONS = None  # type: ignore[assignment]  # pragma: no cover
    CARDINALITY_CURRENT = None  # type: ignore[assignment]  # pragma: no cover
    CARDINALITY_LIMIT = None  # type: ignore[assignment]  # pragma: no cover
...
class CardinalityProtector:
    ...
    def _get_cache(self, label_name: str) -> LRUCache:
        ...
        with self._lock:
            if label_name not in self._caches:
                ...
                if PROMETHEUS_AVAILABLE and CARDINALITY_LIMIT is not None:  # pragma: no branch
    ...
    def set_limit(self, label_name: str, limit: int) -> None:
        ...
        # Recreate cache if exists — old values are intentionally dropped on
        # limit change (the new limit defines a fresh cardinality boundary).
        with self._lock:
    ...
    def protect(
        ...
        if PROMETHEUS_AVAILABLE and CARDINALITY_REJECTIONS is not None:  # pragma: no branch
...
def get_cardinality_protector(config: CardinalityConfig | None = None) -> CardinalityProtector:
    ...
    if _cardinality_protector is None:
        with _protector_lock:
            if _cardinality_protector is None:  # pragma: no cover
                _cardinality_protector = CardinalityProtector(config)  # pragma: no cover
