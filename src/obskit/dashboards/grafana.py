...
class DashboardBuilder:
    ...
    def add_error_budget_panel(self, slo_name: str = ""):
        ...
        query = f'obskit_slo_error_budget_remaining{{slo=~"{slo_name}.*"}}' if slo_name else 'obskit_slo_error_budget_remaining'
    ...
    def add_red_metrics_row(self):
        ...
        self.add_timeseries_panel(
            ...
            queries=[
                {"expr": f'histogram_quantile(0.50, sum(rate({self.service_name}_request_duration_seconds_bucket[5m])) by (le))', "legend": "P50"},
                {"expr": f'histogram_quantile(0.95, sum(rate({self.service_name}_request_duration_seconds_bucket[5m])) by (le))', "legend": "P95"},
                {"expr": f'histogram_quantile(0.99, sum(rate({self.service_name}_request_duration_seconds_bucket[5m])) by (le))', "legend": "P99"},
            ],
            unit="s",
        )
