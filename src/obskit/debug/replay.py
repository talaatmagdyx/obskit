...
class RequestCaptureStorage:
    ...
    async def load(self, capture_id: str) -> Optional[CapturedRequest]:
...
class RequestCapture:
    ...
    def __init__(
        self,
        storage: Optional[RequestCaptureStorage] = None,
    ...
    def _truncate_args(self, args: tuple, kwargs: Dict) -> tuple:
        ...
        truncated_args = tuple(
            f"<truncated: {type(a).__name__}>" if len(str(a)) > 1000 else a
            for a in args
        )
        truncated_kwargs = {
    ...
    async def replay(
        ...
        if not func:
            ...
            if not func:
                return {
                    "success": False,
                    "error": f"Function {key} not in registry"
                }
